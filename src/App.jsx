...
function App() {
  ...
  return (
    <div className="App">
      {React.Children.toArray(["en","ko","chi"].map(lang=><button onClick={() => handleClick(lang)}>{lang}</button>))}
...
{/* 當一個 action creator 回傳一個 function 的時候，這個 function 將會被 Redux Thunk middleware 執行。這個 function 不需要是 pure 的；因此它被允許一些有 side effect 的動作，包括執行非同步的 API 呼叫。這個 function 也可以 dispatch action—像是那些我們之前定義的同步 action。 */}
      ...
      <button  onClick={() => dispatch({type: 'USER_FETCH_REQUESTED', payload: {userId:123}})}>saga dispatch</button>

      <button  onClick={() => dispatch({type: "USER_FETCH_REQUESTED_LATEST", payload: {userId:123}})}>saga  LATEST dispatch</button>
