...
import RouteP from "./pages/RouteP";
import RouteC from "./pages/RouteP/RouteC";
    ...
    <>
      ...
      <Suspense fallback={<div>Home loading...</div>}>
        <Routes>
          ...
          <Route path="/sus" element={<SuspenseTest />} />
          <Route path="/chart-test" element={<ChartTest />} />
          <Route path="/geo" element={<Geo />} />
          <Route path="/xstate" element={<XstateRun />} />
          <Route path="/P/*" element={<RouteP />}>
      ...
      <div className="App">
        {React.Children.toArray(
          ["en", "ko", "chi"].map((lang) => <button onClick={() => handleClick(lang)}>{lang}</button>)
        )}
        <button onClick={() => setBrickColor((pre) => !pre)}>switch brick color</button>
        <button onClick={() => setColor((pre) => !pre)}>color</button>
        <button onClick={() => setData((pre) => [1, ...pre])}>data</button>
        <button onClick={() => dispatch({ type: "INCREMENT" })}>dispatch by +</button>
        <button
          onClick={() => {
        ...
        <button onClick={() => dispatch(callApi("123"))}>call api dispatch</button>

        {/* 直接dispatch fn 可透過HOC的方式，取得dispatch, getState兩個fn */}
        <button onClick={() => dispatch(exampleThunkFunction)}>test Thunk</button>

        <button onClick={() => dispatch({ type: "USER_FETCH_REQUESTED", payload: { userId: 123 } })}>
          saga dispatch
        </button>
